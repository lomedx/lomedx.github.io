...
window.tempPatientContext = {
    ...
    doctorInfo: docInfo
        ...
        if (doctorData?.is_subscribed && !hasAddedRx) {
            ...
            // الزر مقفل، يعرض رسالة تنبيه فقط عند الضغط عليه
            ...
            // زر مقفل لأن الطبيب غير مشترك (هذا الشرط للأطباء غير المشتركين إذا مسحوا الـ QR)
...
window.openMedicineDonation = () => {
    ...
    try {
        ...
        if (funcError) {
            ...
            } else if (funcError.context && funcError.context.error) { errMsg = funcError.context.error; }
            throw new Error(errMsg);
        }
...
window.openMedicineFinder = () => { 
    ...
    // توليد خيارات المدن (مع استبعاد "كل المدن")
    ...
    // === تفعيل أداة الحماية يدوياً (Explicit Rendering) ===
    if (window.turnstile) {
...
window.submitQuestion = async (e) => {
    ...
    if (honeypotField && honeypotField.value !== '') { showToast('تم رفض النشر.', 'error'); return; }
    if (window.askDoctorOpenTime && (Date.now() - window.askDoctorOpenTime < 3000)) { showToast('جاري المعالجة، يرجى الانتظار قليلاً.', 'error'); return; }
